#!/bin/bash
# LFS 13.0-systemd chapter 2.2 version check (from hostreqs.html).
# https://www.linuxfromscratch.org/lfs/view/stable/chapter02/hostreqs.html
#
# Extended: accumulate errors and exit non-zero; verify makeinfo/msgfmt for Glibc.
...
# Not in the book script; required when building Glibc (msgfmt, makeinfo).
for tool in makeinfo msgfmt; do
  if type -p "$tool" &>/dev/null; then
    printf "OK:    %-9s found\n" "$tool"
  else
    printf "ERROR: Cannot find %s (install gettext and texinfo)\n" "$tool"
    fail
  fi
done
...
if [ "$errors" -ne 0 ]; then
  echo ""
  echo "Host check failed with $errors error(s)."
  exit 1
fi
exit 0
